...
firstHalf str | odd n = take (n `div` 2 + 1) str
              | otherwise = take (n `div` 2) str
              where
                n = length str
...
whileRight f x = todo
-- whileRight f x | isRight (f x) = whileRight f (f x)
--                | otherwise = x
...
multiCompose [] = id
multiCompose (f:rest) = f . multiCompose rest
-- multiCompose [] fs = fs
-- multiCompose fxs n = [ n . x | x <- fxs]
------------------------------------------------------------------------------
