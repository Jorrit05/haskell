...
instance Eq Name where
  (==) (Name "") (Name "") = True
  (==) (Name x) (Name y)  = length x == length y && toLower (head x) == toLower (head y) && Name (tail x) == Name (tail y)
...
simplify (RationalNumber x y) = RationalNumber (x `div` commonDivisor) (y `div`commonDivisor)
