...
setName name (MkPerson age pname) = MkPerson age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MkPerson page name) = MkPerson age name
...
data UpDown = UpDownIncrease Int | UpDownDecrease Int
  deriving (Show,Eq)

-- zero is an increasing counter with value 0
zero :: UpDown
zero = UpDownIncrease 0

-- get returns the counter value
get :: UpDown -> Int
get (UpDownIncrease x) = x
get (UpDownDecrease x) = x

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (UpDownDecrease x) = UpDownDecrease (x-1)
tick (UpDownIncrease x) = UpDownIncrease (x+1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownDecrease x) = UpDownIncrease x
toggle (UpDownIncrease x) = UpDownDecrease x
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = map (\(a,b) -> (a+b)/2) (zip (rgb c1) (rgb c2))
rgb (Invert clr) = map (1 -) (rgb clr)
...
toList Empty = []
toList (Pair k v x) = (k,v) : toList x
...
toNat z
  | z < 0 = Nothing
  | z == 0 = Just Zero
  | otherwise = Just (recurseToNat z)
    where
      recurseToNat 0 = Zero
      recurseToNat z = PlusOne (recurseToNat (z-1))
