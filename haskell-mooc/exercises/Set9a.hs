...
workload nExercises hoursPerExercise | totalExerciseTime > 100 = "Holy moly!"
                                     | totalExerciseTime < 10 = "Piece of cake!"
                                     | otherwise = "Ok."
                                     where
                                       totalExerciseTime = nExercises*hoursPerExercise
...
--  countValid ["123453", "123444", "123456"]
-- Given a list of bank note serial numbers (strings), count how many
-- are valid.

countValid :: [String] -> Int
-- countValid xs = length (validList xs)
--                     where
--                       validList [] = []
--                       validList (y:ys) | (y !! 2) == (y !! 4) = y : validList ys
--                                        | (y !! 3) == (y !! 5) = y : validList ys
--                                        | otherwise = validList ys


countValid xs = length $ filter (\x -> (x !! 3) == (x !! 5) || (x !! 2) == (x !! 4)) xs
...
sumSuccess xs | not (any isRight xs) = Left "no data"
              | otherwise = Right $ sum $ map (fromRight 0) xs
...
isOpen :: Lock -> Bool
isOpen (Lock str status) = status

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open code (Lock str status) | code == str = Lock str True
                            | otherwise = Lock str status

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock str status) = Lock str False

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode code (Lock str status) | not status = Lock str status
                                  | otherwise = Lock code status
...
compose [] _ = []
compose ((x,y):xs) ys | isNothing (lookup y ys) = compose xs ys
                      | otherwise = (x, fromJust (lookup y ys)) : compose xs ys
...
permute [] _ = []
permute xs ys =  map snd $ sort $ zip xs ys
