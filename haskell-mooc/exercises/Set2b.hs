...
import Data.List
import Data.Text.Internal.Fusion.Size (smaller)
import GHC.Float.RealFracMethods (roundFloatInt, roundFloatInteger)
...
binomial _ 0 = 1
binomial 0 k = if k > 0 then 0 else k
...
oddFactorial 0 = 1
oddFactorial 1 = 1
oddFactorial x | odd x = x * oddFactorial (x-2)
               | otherwise = oddFactorial (x-1)
...
startCountDown :: Integer -> String
startCountDown 0 = ""
startCountDown x = show x ++ "... " ++ startCountDown (x-1)

countdown :: Integer -> String
countdown x = "Ready! " ++ startCountDown x ++ "Liftoff!"
...
smallestDivisor n | even n = 2
                  | otherwise = head [ x | x <- [2..], n `mod` x == 0]
...
isPrime 0 = False
isPrime 1 = False
isPrime n | smallestDivisor n == n = True
          | otherwise = False
...
biggestPrimeAtMost 0 = 1
biggestPrimeAtMost 1 = 1
